...
;
; A cylon scanner on 12 LEDs (charliplexed)
; along with a PWM wave generation (on PB4)
; that makes the sound of Cylons!
; *********************************************************************************************************************************
...
.org 0
        ...
        reti                                    ;   x  External Interrupt Request 0
        reti                                    ;   x  Pin Change Interrupt Request 0:n
        reti                                    ;   x  Timer/Counter1 Compare Match A
        rjmp set_pwm_dc                         ; Timer/Counter1 Overflow
        rjmp scan                               ; Timer/Counter0 Overflow
        reti                                    ;   x  EEPROM Ready
        reti                                    ;   x  Analog Comparator
        reti                                    ;   x  ADC Conversion Complete
        reti                                    ;   x  Timer/Counter1 Compare Match B
        rjmp pwm_mid                            ; Timer/Counter0 Compare Match A
        rjmp pwm_end                            ; Timer/Counter0 Compare Match B
        reti                                    ;   x  Watchdog Time-out
        reti                                    ;   x  USI Start
        reti                                    ;   x  USI Overflow
...
wave_table:                             ; 512 samples of a sinewave (amplitude: 0 - 255)
